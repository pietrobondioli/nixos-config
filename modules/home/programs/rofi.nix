{ config, pkgs, ... }: {
  programs.rofi = {
    ...
    theme = let inherit (config.lib.formats.rasi) mkLiteral; in {
      "*" = { bg = mkLiteral "#1e1e2eff"; bg-alt = mkLiteral "#313244ff"; bg-selected = mkLiteral "#45475aff"; fg = mkLiteral "#cdd6f4ff"; fg-alt = mkLiteral "#7f849cff"; border = mkLiteral "#89b4faff"; border-alt = mkLiteral "#b4befeff"; background-color = mkLiteral "transparent"; text-color = mkLiteral "@fg"; font = "JetBrainsMono Nerd Font 12"; margin = 0; padding = 0; spacing = 0; };
      ...
      "element normal.urgent" = { background-color = mkLiteral "#f38ba8ff"; text-color = mkLiteral "@bg"; };
      "element normal.active" = { background-color = mkLiteral "#a6e3a1ff"; text-color = mkLiteral "@bg"; };
      "element selected.normal" = { background-color = mkLiteral "@bg-selected"; text-color = mkLiteral "@border"; };
      "element selected.urgent" = { background-color = mkLiteral "#f38ba8ff"; text-color = mkLiteral "@bg"; };
      "element selected.active" = { background-color = mkLiteral "#a6e3a1ff"; text-color = mkLiteral "@bg"; };
